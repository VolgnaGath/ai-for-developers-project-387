...
- **F2. Фильтр диапазона дат в «Предстоящих встречах»** — админка `/admin/bookings`: контролы `from`/`to`; backend `listBookings` уже поддерживает обе границы — правки только на фронте (+ MSW/e2e). Реализовано агентом в issue #30, PR #32.
...
- [x] Артефакт 3: PR агента по задаче из §5 (F2, issue #30) + доработка после ревью-комментария `/oc`, CI зелёный. PR #32 (ветка `opencode/issue30-…`, автор `github-actions[bot]`): feat-коммит + `test`; после ревью-комментария `/oc` на строку — коммит агента `feat: add reset button…`, CI перепроверен зелёным, squash-merge. **Отклонение**: задача взята не B1 (уже исправлена человеком в PR #21), а F2 из бэклога §5. Побочно: первый push агента падал на `git push` из-за upstream-бага anomalyco/opencode v1.18.18 (`configureGit` пропускается при `use_github_token: true`) — в `opencode.yml` добавлен шаг «Configure git credentials» (PR #31, разрешено человеком в обход запрета AGENTS.md на правку workflows).
...
- [x] Финальная проверка интеграции: триггеры — только `/oc`/`/opencode` от OWNER/MEMBER/COLLABORATOR, боты отсечены `[bot]`-фильтром (подтверждено 19 `skipped`-прогонами, включая self-trigger ответа агента); permissions минимальные (интерактив `issues`/`contents`/`pull-requests: write`, triage/lighthouse `issues: write` + `contents: read`); schedule Lighthouse раз в сутки (`0 1 * * *`). Сценарии подтверждены фактами: triage (#36/#35/#30) → PR агента (#37, #32) → ревью через `/oc` (#37: переименование заголовка по Conventional Commits, правка «слоты закончились» на нерабочих днях с frozen-clock e2e) → schedule (15.08 issue #28 «perf», 16.08 без регрессии, спама нет). Самооценка: с первого раза решено больше задач (триажи, фикс #36, фича F2, детект регрессии); итерации — тулинг/upstream (PR #19/#31/#24-26, 3 упавших прогона 14.08) и конвенции (заголовок PR, формулировка пустого состояния), не по логике.
